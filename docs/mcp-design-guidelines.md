...
Sessions:
- TOKEN-ECONOMY: Strategies for reducing context waste (summaries, excerpts).
...
* Reading dependency folders (.venv).  
...
* **metadata:** Technical context (e.g., token-saving strategies).  
...
```json
{
  ...
  "files": [
    {
      "path": "src/auth.py",
      "reason": "Contains the main login login.",
      "line_range": { "start": 10, "end": 50 }
    }
  ],
  "warnings": [
    { "type": "large_file_skipped", "message": "auth.log was skipped." }
  ],
...
* Do not index secrets, binary files, or ignored paths.  

---

## Caching for Token Efficiency

Caching is a first-class feature. We cache project tree snapshots, file summaries, and symbol indexes to avoid  
paying the same token cost twice.  
...
* **Good:** Return a compact project map and read only necessary excerpts.  

### File Reading
* **Bad:** Return a 2,000-line file because one function may be relevant.  
* **Good:** Return the target function, its imports, and related definitions.  

### Command Output
* **Bad:** Run tests and paste the full 500-line raw output.  
* **Good:** Summarize results, include failing test names and error excerpts.  
